    ...
    <div className='mt-10 p-5'>
      ...
      <div className='grid grid-cols-2 gap-5 mt-6'>
        {quiz?.options.map((option, index)=>(
          ...
          key={index} className={`mt-5 w-full border rounded-full p-3 px-4 
          text-center text-lg hover:bg-gray-200 cursor-pointer
          ${selectedOption === option && 'bg-primary text-white hover:bg-primary' }`} 
              variant='outline' color='primary'>{option}</h2>
        ))}
